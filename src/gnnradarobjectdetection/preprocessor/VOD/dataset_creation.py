...
from gnnradarobjectdetection.preprocessor.VOD import conversion
...
from gnnradarobjectdetection.preprocessor.VOD.configs import VODDatasetConfiguration, VODSplitConfiguration
...
class VODGraphDataset(Dataset):
    def __init__(self,
                 root: str,
                 graph_config: GraphConstructionConfiguration,
                 dataset_config: VODDatasetConfiguration,
                 transform=None,
                 pre_transform=None,
                 pre_filter=None):
    ...
    def filter_bounding_boxes(nusc, boxes):
        ...
        boxes = [box for box, annotation in zip(boxes, annotations) if
                 annotation['num_lidar_pts'] + annotation['num_radar_pts'] > 0]
    ...
    def get_sensor_points(self, nusc, sample: dict, sensor: str) -> np.ndarray:
        ...
        sensor_points, timestamps = nuscenes.RadarPointCloud.from_file_multisweep(nusc, sample, chan=sensor,
                                                                                  ref_chan=sensor,
    ...
    def get_VODlabels(self, number_of_radar_points: int, points: np.ndarray, data: np.ndarray, trMatrix: np.ndarray) -> np.ndarray:
        # VOD数据集中使用KITTI格式的label
        # KITTI格式label
        # read the first label, 00000.txt
        #data = np.loadtxt("/home/sdd_FTP/neousys/project/RadarGNN/data/datasets/VOD/raw/label_2/01201.txt", dtype=str)

        # number_of_radar_labels = data.shape[0]
        sensor_labels = data[:, 0]
        # Get bounding boxes
        boxes = data[:, 8:15]
        # 转成float，3D包围框信息，包含dimensions和location
        boxes = np.array(boxes).astype(np.float32)
        boxes_label = data[:, 0]
        print(boxes_label)
        # 转换label为id号：
        for idx, label in enumerate(boxes_label):
            if label == 'Car':
                data[idx, 0] = 1
                ...
                data[idx, 0] = 5
            elif label == 'bicycle_rack':
                data[idx, 0] = 6
            elif label == 'human_depiction':
                data[idx, 0] = 7
            elif label == 'moped_scooter':
                data[idx, 0] = 8
            elif label == 'motor':
                data[idx, 0] = 9
            elif label == 'ride_other':
                data[idx, 0] = 10
            elif label == 'vehicle_other':
                data[idx, 0] = 11
            elif label == 'ride_uncertain':
                data[idx, 0] = 12
            elif label == 'truck':
                data[idx, 0] = 13
            elif label == 'DontCare':
        ...
        # # quasi 2d radar data).这里是因为neuscenes毫米波雷达没有z轴高度信息。只有距离x轴坐标和水平y轴坐标信息。
        # points[2, :] = 0.0
        #
        ...
        #     points_in_box = utils.extended_points_in_box(box=box, points=points[:3, :], wlh_factor=self.wlh_factor, wlh_offset=self.wlh_offset, use_z=False)
        #
        #     # nuscenes中毫米波雷达点是没有类别标签的，因此把3D包围框的类别标签赋值给了雷达点，也就是每个点的sensor_label
        #     # Assign class label to all points within the bounding box.
        #     sensor_labels[points_in_box] = box.label

        for index, box in enumerate(boxes):
            # Determine if points are within the bounding box，如果是VOD，Z轴坐标有效
            ...
            points_in_box, BBox_new = utils.extended_points_in_3Dbox(box=box, points=points[:3, :],
                                                                     transMatrix=trMatrix, wlh_factor=self.wlh_factor,
                                                                     wlh_offset=self.wlh_offset, use_z=True)

            # nuscenes中毫米波雷达点是没有类别标签的，因此把3D包围框的类别标签赋值给了雷达点，也就是每个点的sensor_label
            # Assign class label to all points within the bounding box.
            # sensor_labels[points_in_box] = boxes_label[index]
            sensor_labels.reshape(sensor_labels.shape[0], 1)[points_in_box] = boxes_label[index]
            boxes[index, 3:6] = BBox_new
    ...
    def get_labels(self, nusc, sample: dict, sensor: str, points: np.ndarray) -> np.ndarray:
        ...
        for box in boxes:
            ...
            # nuscenes中毫米波雷达点是没有类别标签的，因此需要把3D包围框的类别标签赋值给了点
            # Assign class label to all points within the bounding box.
            sensor_labels[points_in_box] = box.label
    ...
    def create_graph_data(self, geometric_graph, labels, bounding_boxes, point_cloud) -> Data:
        ...
        # Vel = torch.tensor(point_cloud.V_cc_compensated, dtype=torch.float32) #for neuscenes
        Vel = torch.tensor(point_cloud.vr_compensated, dtype=torch.float32)
    ...
    def processed_single_VOD_sample(self, frame_data, path_to_destination: str = None) -> None:
        ...
        # points = np.empty(shape=(frame_data.radar_data.shape[1] + 1, 0))

        # points = frame_data.radar_data.T
        ...
        # for show graph
        # geometric_graph.show()

        # Convert bounding boxes
        # bounding_boxes = conversion.convert_bounding_boxes(self.dataset_config, point_cloud, boxes, wlh_factor=self.wlh_factor, wlh_offset=self.wlh_offset)
    ...
    def process_single_sample(self, path_to_destination: str = None) -> None:
        """ Prepares a single nuScenes sample for model training or evaluation.
    ...
    def process(self) -> None:
        ...
        path_to_VOD = f"{self.root}/raw/"
        path_to_destination = f"{self.root}/processed/train/"
        os.mkdir(path_to_destination)

        data_path = path_to_VOD + "radar/ImageSets/"

        filelist = np.loadtxt(data_path + "train.txt", dtype=str)
        dataset_split = VODSplitConfiguration(version=self.dataset_config.version)
        self.dataset_split_config = dataset_split

        for idx, filename in enumerate(filelist):
            filename = "01201"
            # for vod sample testing
            kitti_locations = KittiLocations(root_dir=path_to_VOD, output_dir="example_output", frame_set_path="",
                                             pred_dir="", )
            # frame_data = FrameDataLoader(kitti_locations=kitti_locations, frame_number="01201")
            # data = np.loadtxt("/home/sdd_FTP/neousys/project/RadarGNN/data/datasets/VOD/raw/label_2/01201.txt", dtype=str)
            frame_data = FrameDataLoader(kitti_locations=kitti_locations, frame_number=filename)
            data = np.loadtxt(path_to_VOD + "label_2/" + filename + ".txt", dtype=str)
            ...
            boxes = data[:, 8:15]
            # 转成float，3D包围框信息，包含dimensions和location
            original_boxes = np.array(boxes).astype(np.float32)

            # labels, FrameLabels = FrameLabels(frame_data.raw_labels)
            ...
            # create_data_set_description(self.graph_config, self.dataset_config, self.dataset_split_config, path_to_config)
            ...
            # self.processed_single_VOD_sample(frame_data,path_to_destination)
            ...
            # 转换成齐次坐标
            # #from https://github.com/tudelft-iv/view-of-delft-dataset/blob/main/2_frame_transformations.ipynb
            transforms = FrameTransformMatrix(frame_data)

            # radar points
            points = frame_data.radar_data.T
            transMatrix = transforms.t_camera_radar  # from camera to radar

            # lidar points
            # points = frame_data.lidar_data.T
            # transMatrix = transforms.t_camera_lidar  # from camera to lidar

            points_nums = points.shape[1]

            # Get sensor point cloud lables and bounding boxes
            # labels, boxes = self.get_labels(nusc, sample, sensor='LIDAR_TOP', points=points)

            # Get VOD radar point cloud labels and bouding boxes
            # labels, boxes = self.get_VODlabels(points_nums, points, transMatrix)
            # points, 3xN
            labels, new_boxes = self.get_VODlabels(points_nums, points, data, transMatrix)
            ...
            # for show graph
            #geometric_graph.show()
            geometric_graph.show3D()

            # Convert bounding boxes
            bounding_boxes = conversion.convert_bounding_boxes(self.dataset_config, point_cloud, original_boxes,
                                                               transMatrix, wlh_factor=self.wlh_factor,
            ...
            # Save graph
            # save_name = f"{path_to_destination}/graph_{sample['timestamp']}_{sample['scene_token']}_{sample['token']}.pt"
            save_name = path_to_destination + filename + ".pt"
            torch.save(graph, save_name)
...
def create_data_set_description(graph_config: GraphConstructionConfiguration, dataset_config: VODDatasetConfiguration,
                                split_config: VODSplitConfiguration, path_to_destination: str) -> None:
    """
